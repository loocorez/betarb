...
class c_betpix365:
    ...
    def get_all_campeonatos(self):
        ...
        data = get_url_json(url=url, headers=headers)
        # sql_string = ''
        # _sports = {}
        # _paises = {}
        # _campeonatos = {}
        ...
        for esporte in data['data']:
            ...
            #     self.sports[esporte["stSURL"]] = {'name': esporte["stN"].strip(), 'id': esporte["xid"]}
            ...
            #     sqlxx = f'INSERT INTO sit_sports (id_site, id_site_sport, site_sport_name, id_ind_sport, slug) VALUES ({self.id_site},"{esporte["xid"]}","{esporte["stN"].strip()}",{_sports[esporte["stSURL"]]}, "{esporte["stSURL"]}") ON DUPLICATE KEY UPDATE site_sport_name="{esporte["stN"].strip()}", slug="{esporte["stSURL"]}";'
            #     self.conexao.bd(sqlxx, fetch=False)
            #
            # if not esporte['xid'] in self.all_campeonatos:
            #     self.all_campeonatos[esporte['xid']] = {'esporte': esporte['stN'].strip(),
            ...
            for pais in esporte['cs']:
                if not pais["cSURL"] in self.paises:
                    ...
                    # sqlxx = f'INSERT INTO sit_paises (id_site, id_site_pais, site_pais_name, id_ind_pais, slug) VALUES ({self.id_site},"{pais["xid"]}","{pais["cN"].strip()}",{_paises[pais["cSURL"]]},"{pais["cSURL"]}") ON DUPLICATE KEY UPDATE site_pais_name="{pais["cN"].strip()}", slug="{pais["cSURL"]}";'
                    # self.conexao.bd(sqlxx, fetch=False)
                # if not pais['xid'] in self.all_campeonatos[esporte['xid']]['paises']:
                #     self.all_campeonatos[esporte['xid']]['paises'][pais['xid']] = {'nome': pais['cN'].strip(),
                #                                                                     'cSURL': pais['cSURL'],
                #                                                                     'camp': {}}
                for camp in pais['sns']:
                    if not camp["seaSURL"] in self.campeonatos:
    ...
    def get_lista_eventos(self, id:str):
        ...
        response = post_url_json(url, headers, input_data)
        print('GET', url)
        _eventos = {}
    ...
    def teste(self):
        ...
        xxx = post_url_json(url, headers,data)
        return xxx
